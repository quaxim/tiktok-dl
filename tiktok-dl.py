...
def getUserByUsername(username, retry=1):
    try:
        ...
        if retry < 4:
            print("! Unexpected response by API endpoint, retrying ({:d}).".format(retry))
            print()
            time.sleep(0.5)
            retry += 1
    ...
    try:
        ...
        for videoObj in response.get("aweme_list"):
            ...
            if awemeDownload(video=video):
                print("> Downloading new video {:s}.mp4 ({:d}/{:d})".format(video.awemeId, count, total))
            else:
                print("! Skipped existing file {:s}.mp4 ({:d}/{:d})".format(video.awemeId, count, total))
        ...
        if hasmore:
            ...
            print()
            print("> Finished downloading ({:d}/{:d}) videos.".format(count, total))
            if count < total:
